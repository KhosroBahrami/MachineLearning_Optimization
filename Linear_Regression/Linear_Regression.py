...
# Implementation of SGD for Linear Regression 
...
def main():
    ...
    W,w0 = our_SGD(train_data, learning_rate=0.01, niter=1000, batch_size=1)
    y_pred_our_sgd = predict(x_test, W, w0)
    print('Mean Squared Error (Stochastic Gradient Descent) :',mean_squared_error(y_test, y_pred_our_sgd))
    ...
    W,w0 = our_SGD(train_data, learning_rate=0.01, niter=1000, batch_size=train_data.shape[0])
    y_pred_our_sgd = predict(x_test, W, w0)
    print('Mean Squared Error (Batch Gradient Descent) :',mean_squared_error(y_test, y_pred_our_sgd))
